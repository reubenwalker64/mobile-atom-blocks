...
registerBlockType( 'mobile-atom-blocks/example-03-toc-card-esnext', {
	title: __( 'Article Table of Contents Card (esnext)', 'mobile-atom-blocks' ),
	icon: 'index-card',
	category: 'layout',
	attributes: {
		title: {
	...
	edit: ( props ) => {
		...
		return (
			<div className={ className }>
				<RichText
					tagName="h2"
					placeholder={ __( 'Write Article Table of Contents title…', 'mobile-atom-blocks' ) }
					value={ title }
					...
					<MediaUpload
						...
						render={ ( { open } ) => (
							<Button className={ mediaID ? 'image-button' : 'button button-large' } onClick={ open }>
								{ ! mediaID ? __( 'Upload Image', 'mobile-atom-blocks' ) : <img src={ mediaURL } alt={ __( 'Upload toc Image', 'mobile-atom-blocks' ) } /> }
							</Button>
						) }
					/>
				</div>
	...
	save: ( props ) => {
		...
		return (
			<div className={ className }>
				...
				{
					mediaURL && (
						<img className="toc-image" src={ mediaURL } alt={ __( 'Article Table of Contents Image', 'mobile-atom-blocks' ) } />
					)
				}
